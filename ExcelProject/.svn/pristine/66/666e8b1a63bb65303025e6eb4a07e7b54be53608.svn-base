 ...
 */
public class IRChassisCoveredCardsSheetHandler extends DefaultHandler {

//	protected static final Logger logger = LoggerFactory.getLogger(IRChassisCoveredCardsSheetHandler.class);
	...
	public IRChassisCoveredCardsSheetHandler(SharedStringsTable sst) {
		this.sst = sst;
	}
	...
	public void startElement(String uri, String localName, String name,
		...
		if (name.equals("c")) {
			...
			try {
				...
				matcher = Pattern.compile(ExcelConstants.CHASSIS_COVERED_CARDS_SHEET_DATA_COLUMN_REGEX)
...
//				logger.debug("THE COLUMN NAME IS: " + currentColumName+ " ROW NUMBER IS: " + currentRowNumber);
				if (currentRowNumber >= ExcelConstants.CHASSIS_COVERED_CARDS_SHEET_DATA_START_ROW_NUMBER
...
//				logger.debug(lastContents);
				...
				if (currentColumName.compareToIgnoreCase("B") == 0) {
					...
					// adding dummy vlaues
					aCoveredDevicesDTO.setServicelevel("No data in Chassis Covered cards");
					aCoveredDevicesDTO.setInstallsiteid("No data in Chassis Covered cards");
					aCoveredDevicesDTO.setInstallsitecustomer("No data in Chassis Covered cards");
