 ...
 */
public class SNTCAllContractsSheetHandler extends DefaultHandler {
	
//	protected static final Logger logger = LoggerFactory.getLogger(SNTCAllContractsSheetHandler.class);
	...
	public SNTCAllContractsSheetHandler(SharedStringsTable sst) {
		this.sst = sst;
	}
	...
	public void startElement(String uri, String localName, String name,
		...
		if (name.equals("c")) {
			...
			matcher = Pattern.compile(ExcelConstants.SNTC_All_CONTRACT_SHEET_DATA_COLUMN_REGEX).matcher(attributes.getValue("r"));
			matcher.find();
			currentColumName = matcher.group();
//			logger.debug("THE COLUMN NAME IS: " + currentColumName	+ " ROW NUMBER IS: " + currentRowNumber);
			if (currentRowNumber >= ExcelConstants.SNTC_All_CONTRACT_SHEET_DATA_START_ROW_NUMBER && (!currentColumName.isEmpty())) {
				processCell = true;
...
//		logger.debug("THE COLUMN NAME IS: " + currentColumName	+ " ROW NUMBER IS: " + currentRowNumber);
//		logger.debug("contents: "+lastContents+ " Name of the tag: "+name);
		if (processCell) {
...
//				logger.debug(lastContents);
				...
				if (currentColumName.compareToIgnoreCase("A") == 0) {
				...
				} else if (currentColumName.compareToIgnoreCase("B") == 0) {
					...
					aContractDTO.setContractStartDate(Util.convertDate(lastContents,ExcelConstants.DATE_FORMAT_YYYY_MM_DD));

				} else if (currentColumName.compareToIgnoreCase("G") == 0) {
					aContractDTO.setContractEndDate(Util.convertDate(lastContents,ExcelConstants.DATE_FORMAT_YYYY_MM_DD));
