 ...
 */
public class IRCoveredItemsSheetHandler extends DefaultHandler {

//	protected static final Logger logger = LoggerFactory.getLogger(IRCoveredItemsSheetHandler.class);
	...
	public IRCoveredItemsSheetHandler(SharedStringsTable sst) {
		this.sst = sst;
	}
		...
		if (name.equals("c")) {
			...
			try {
				...
				matcher = Pattern.compile(
						ExcelConstants.COVERED_ITEMS_SHEET_DATA_COLUMN_REGEX)
...
//						+ " ROW NUMBER IS: " + currentRowNumber);
				if (currentRowNumber >= ExcelConstants.COVERED_ITEMS_SHEET_DATA_START_ROW_NUMBER
...
//				logger.debug(lastContents);
				...
				} else if (currentColumName.compareToIgnoreCase("P") == 0) {
